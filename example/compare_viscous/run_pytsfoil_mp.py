...
def plot_all_mach_distributions(results, output_dir):
    ...
    for i, result in enumerate(successful_results):
        ...
        ax.plot(mach_data['xx'], mach_data['mau'], 
                linewidth=2, color=color, linestyle=lines[nwdge], markersize=4,
                label=f"AoA={alpha}°, NWDGE={nwdge}")
...
if __name__ == "__main__":
    ...
    cases = []
    alphas = [0.0, 1.0]  # Different angles of attack
    nwdges = [0, 1, 2]  # Different viscous wedge types
    
    for i, alpha in enumerate(alphas):
        for j, nwdge in enumerate(nwdges):
            ...
            cases.append({
                'case_name': f'alpha_{alpha}_nwdge_{nwdge}',
